...
//
// Added octave switch -John Coxworth 2020.8
...
//Octave counter, start on 2nd octave
int octcnt = 2;
//Will be octave counter multiplied by 12
int octalpha = 0;
//octave up button is on 12
const int  octuppin = 12;
const int octdownpin = 13;
int octupstate = 0;
int lastoctupstate = 0;
int octdownstate = 0;
int lastoctdownstate = 0;
...
unsigned int tone_inc[88];
void settones() {
  for (byte i=0; i<88; i++){
    tone_inc[i]= 55.0 * pow(2.0, ( (i-9) / 12.0)) * 65536.0 / (16000000.0/512) + 0.5;
  }
  //set starting octave
  octcnt = 2;
  octalpha = octcnt * 12;
...
void loop() {
  ...
  //conditionals for octave buttons
  ...
  if (keypressed != nokey) {
    phase[nextch]=0;
    amp_base[nextch] = 64;
    //this is where we can add a number to the octave, octalpha is a octcnt (counter integer) multiplied by 12
    inc_base[nextch] = tone_inc[keypressed+ octalpha];
    iADSR[nextch] = 1;
    FMphase[nextch]=0;
    FMexp[nextch]=0xFFFF;
...
//Octave buttons
  octupstate = digitalRead(octuppin);
  octdownstate = digitalRead(octdownpin);
  
  if (octupstate != lastoctupstate)
  {
    if (octupstate == HIGH)
    {   
        if (octcnt<5 ) 
  ...
  if (octdownstate != lastoctdownstate)
  {
